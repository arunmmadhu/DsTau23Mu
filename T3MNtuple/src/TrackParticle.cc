...
TrackParticle::TrackParticle():
  SimpleParticle(TMatrixT<double>(NHelixPar,1),TMatrixTSym<double>(NHelixPar),0,0,0),
  mass(0)
{
}

TrackParticle::TrackParticle(const TrackParticle& another):
  SimpleParticle(another.getParMatrix(), another.getCovMatrix(), another.PDGID(), another.Charge(), another.BField()),
  mass(another.Mass())
{
}

...
{
  cout << "The branch \"" << br->GetName() << "\" takes " << sizeOnDisk(br,true)/1048576. << " Mb on disk\n";
  size_t n = br->GetListOfBranches()->GetEntries();
  for( size_t i = 0; i < n; ++ i ) {
    TBranch *subbr = dynamic_cast<TBranch*>(br->GetListOfBranches()->At(i));
    cout << "  It's sub-branch \"" << subbr->GetName() << "\" takes " << sizeOnDisk(subbr,true)/1048576. << " Mb on disk\n";
  }
}

void printTreeSummary(TTree *t)
{
  cout << "The TTree \"" << t->GetName() << "\" takes " << sizeOnDisk(t)/1048576. << " Mb on disk\n";
  size_t n = t->GetListOfBranches()->GetEntries();
  for( size_t i = 0; i < n; ++ i ) {
    TBranch *br =dynamic_cast<TBranch*>(t->GetListOfBranches()->At(i));
    cout << "  It's branch \"" << br->GetName() << "\" takes " << sizeOnDisk(br,true)/1048576. << " Mb on disk\n";
  }
}
